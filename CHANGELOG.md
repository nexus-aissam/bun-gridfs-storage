...
- **Bucket Name**: Now correctly uses `fileConfig.bucketName` for uploads instead of default "fs" bucket
- Files are now saved to the correct GridFS collection (e.g., `uploads.files` instead of `fs.files`)
